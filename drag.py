...
class DraggablePoint:
    ...
    def __init__(self, parent, x=0.1, y=0.1, size=0.001):
        ...
        print(parent.fig.axes[0].patch)
        self.press = None
        self.background = None
        self.connect()
        self.line = None
        print("ile punktów:", len(parent.list_points))
        ...
        if self.parent.list_points:
            ...
            print(self.parent.list_points[-1].x, self.parent.list_points[-1].y, self.x, self.y)
...
'''        if len(self.parent.list_points) % 4 == 0:
...
'''            if len(self.parent.list_points) == 3:
                print("Mamy 4 punkty!")
                # print(self.parent.list_points[-3].x, self.parent.list_points[-3].y)
                line_x = [self.parent.list_points[-3].x, self.x]
                line_y = [self.parent.list_points[-3].y, self.y]

                self.line = Line2D(line_x, line_y, color='r', alpha=1)
                parent.fig.axes[0].add_line(self.line)
                self.parent.list_points[-3].line = self.line'''